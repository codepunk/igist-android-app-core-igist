 ...
 */
fun LocalApi.toApi(): Api = Api(bookId, apiVersion, igistMode, surveyLink)

/**
 * Converts a nullable [LocalApi] to a nullable domain [Api].
 */
fun LocalApi?.toApiOrNull(): Api? = this?.let {
    Api(it.bookId, it.apiVersion, it.igistMode, it.surveyLink)
}
...
fun RemoteApi.toLocalApi(bookId: Long): LocalApi =
    LocalApi(bookId, apiVersion, igistMode, surveyLink)
...
fun RemoteApi?.toLocalApiOrNull(bookId: Long): LocalApi? = this?.let {
    LocalApi(bookId, it.apiVersion, it.igistMode, it.surveyLink)
}
